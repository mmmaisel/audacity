...
_name=Audacity
pkgbase=audacity
pkgname=(audacity-compressor2 audacity-docs)
pkgver=3.1.3
pkgrel=1
...
package_audacity-compressor2() {
  ...
  install -vDm 644 $_name-$pkgver-Source/lib-src/libnyquist/nyquist/license.txt "$pkgdir/usr/share/licenses/$pkgbase/LICENSE.nyquist"
  ...
  (
  cd "$pkgdir/usr/lib/$pkgbase/modules"
  for _lib in *.so; do
    chrpath --delete "$_lib"
    chmod 755 "$_lib"
  done
